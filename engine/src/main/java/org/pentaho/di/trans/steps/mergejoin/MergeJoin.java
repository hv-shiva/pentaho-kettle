...
public class MergeJoin extends BaseStep implements StepInterface {
  ...
  private JSONObject previousKeysAction( Map<String, String> queryParams ) throws KettleException {
    ...
    if ( StringUtils.isBlank( index ) || !StringUtils.isNumeric( index ) ) {
      throw new KettleException( BaseMessages.getString( PKG, "MergeJoinDialog.ErrorGettingFields.DialogTitle" ) );
    }
    try {
      JSONArray keysList = new JSONArray();
      String[] keys = getPreviousStepKeys( Integer.parseInt( index ) );
      if ( keys != null ) {
  ...
  private String[] getPreviousStepKeys( Integer index ) throws KettleException {
    ...
    try {
      ...
      StepMeta stepMeta = infoStreams.get( index ).getStepMeta();
      if ( stepMeta != null ) {
